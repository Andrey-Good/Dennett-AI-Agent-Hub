...
This is an AI Core service for the Dennett AI platform. It provides a comprehensive API for managing AI model lifecycles, including searching models from Hugging Face Hub, downloading models with progress tracking, and managing local model storage.
...
# Run with coverage
python -m pytest tests/test_huggingface_service.py --cov=ai_core.logic.huggingface_service
...
- `AI_CORE_API_TOKEN`: Secure API token for authentication
...
### File Organization
- API routes in `ai_core/api/hub.py` and `ai_core/api/local.py`
- Service classes in `ai_core/logic/`
- Data models in `ai_core/models.py`
- Configuration in `ai_core/config/`
