...
router = APIRouter(prefix="/hub", tags=["Hugging Face Hub"])


@router.get("/search", response_model=List[ModelInfoShort])
...
@router.get("/model/{author}/{model_name}", response_model=ModelInfoDetailed)
...
@router.get(
    "/model/{author}/{model_name}/gguf-providers", response_model=List[GGUFProvider]
)
