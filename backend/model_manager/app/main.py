...
# ƒÓ·‡‚ËÚ¸ ÍÓÌÂ‚Û˛ Ô‡ÔÍÛ ‚ sys.path
sys.path.insert(0, str(Path(__file__).parent.parent.parent))

# “ÂÔÂ¸ ËÏÔÓÚ˚ ‡·ÓÚ‡˛Ú
from model_manager.core.config.settings import config, validate_directories
from model_manager.core.models import (
    ModelInfoShort, ModelInfoDetailed, GGUFProvider,
    DownloadRequest, DownloadResponse, DownloadStatus, DownloadState,
    ImportRequest, LocalModel, SearchFilters, ErrorResponse,
    TaskType, LicenseType, SortType, ImportAction
)
...
@app.get("/hub/model/{author}/{model_name}/gguf-providers", response_model=List[GGUFProvider])
...
async def start_download(
    ...
    try:
        ...
        logger.info(f"Download initiated: {download_id} for {request.repo_id}/{request.filename}")
...
async def get_download_status_stream(
    ...
    try:
        ...
        async def event_stream():
            try:
                async for update in download_manager.subscribe_to_updates(subscriber_id):
                    yield update
...
async def delete_local_model(
    model_id: str,
    force: bool = Query(False, description="Force deletion even if file appears in use"),
    #token: str = Depends(verify_token),#
    local_storage: LocalStorage = Depends(get_local_storage)
):
